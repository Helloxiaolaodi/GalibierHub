...
- **Security.txt**: RFC 9116 vulnerability disclosure file at `/.well-known/security.txt`, with `/security` policy and `/acknowledgments` pages.
 ...
 - **Loading System**: A weighted loading feedback system with three tiers. Heavy-duty operations (initial site boot, complex cohort queries, cluster batch downloads) trigger a full-screen `GalibierLoader` climbing animation featuring a detailed alpine scene (snow-capped peaks, forest ridges, cliff face, mountain road with yellow dashed center line), a detailed cyclist SVG with animated legs and wheels, real-time altitude HUD (0-2,642m), and Chinese-labeled telemetry dashboard. Lightweight page transitions use `nextjs-toploader` (2px teal progress bar). The `LoadingContext` implements a 250ms grace period and 2.0s minimum display time to prevent flicker.
...
[Back to top](#readme-top)
 ...
 - A new `GalibierLoader` climbing animation replaces all skeleton screens. The loader now renders a full alpine scene with layered snow peaks, forest ridges, cliff face, and a mountain road with a yellow dashed center line. A detailed cyclist SVG with animated legs, crank arms, and spinning wheels climbs the road. The telemetry dashboard displays Chinese labels ("海拔高度" for altitude, "爬坡进度" for progress) and a D 902 · COL DU GALIBIER route badge. It is used in three heavy-duty scenarios: initial site boot (splash screen), complex cohort querying in Records, and cluster batch download script generation in Downloads.
 ...
 - The `LoadingContext` implements a 250ms grace period (if the operation completes within 250ms, the loader never appears) and a 2.0s minimum display time (once shown, the loader stays visible for at least 2.0s to prevent flicker). The `isLoading` state starts as `true` so the splash screen renders in the server HTML before any page content, and the loader DOM is placed before children in the provider.
 - The `skeleton-screen.tsx` component has been removed from all page imports; skeleton screens are no longer used anywhere in the application. The `src/app/loading.tsx` file has also been deleted — the splash screen is now managed entirely by `LoadingContext` with `isLoading` defaulting to `true`.
 - The Overview page (`page.tsx`) no longer calls `showLoading(undefined)` on mount; the splash is already visible from the context default. It fetches `/api/stats` and calls `hideLoading()` after a 2500ms timeout.
