...
<div className="min-h-[150px] max-h-[400px] overflow-y-auto px-4 py-3 text-sm text-gray-700 prose prose-sm max-w-none">{text.trim() ? renderMarkdown(text) : <span className="text-gray-400 italic">Nothing to preview</span>}</div>
        ...
        <div className="flex items-center justify-between border-t border-gray-100 bg-gray-50/50 px-4 py-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={handleSubmit} disabled={submitting||!text.trim()} className="rounded-lg bg-slate-800 px-4 py-2 text-sm font-medium text-white hover:bg-slate-900 disabled:bg-blue-300 disabled:cursor-not-allowed transition-colors">{submitting?"Posting...":"Reply"}</button>
          </div>
        </div>
      </div>
    </div>
...
export default function DiscussionDetailPage() {
 ...
 const handleDeletePost = useCallback(async (postId: string, isDiscussion: boolean) => {
  ...
  const handleMarkResolved = useCallback(async (resolved: boolean) => {
    ...
    await fetch("/api/feedback", { method: "PATCH", headers: { "Content-Type": "application/json", Authorization: "Bearer " + token }, body: JSON.stringify({ id: entry.id, creator_reply: resolved ? "\u2705 This discussion has been marked as resolved by the GalibierHub Team." : null }) });
    fetchData();
  }, [entry, fetchData]);
