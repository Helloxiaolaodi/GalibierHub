      ...
      {open && (
        ...
        >
          <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100">
            ...
            {unreadCount > 0 && (
              <button onClick={markAllRead} className="text-xs text-slate-600 hover:text-slate-800 font-medium">
                Mark all read
              </button>
            )}
          </div>
          <div className="max-h-[360px] overflow-y-auto">
            ...
            {notifications.map(notif => (
              <Link
                ...
                className={"flex items-start gap-3 px-4 py-3 hover:bg-gray-50 transition-colors border-b border-gray-50 last:border-0 " + (!notif.is_read ? "bg-slate-50/50" : "")}
              >
                <div className="flex-shrink-0 mt-0.5">
                  {!notif.is_read && <div className="h-2 w-2 rounded-full bg-slate-500" />}
                </div>
